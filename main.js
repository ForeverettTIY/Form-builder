...
let formDatalist = document.querySelector ("#fields");
 ...
 for( let i = 0; i < formData.length; i++ ){
   ...
   let userInfo = formData[j];
    let languagebox = document.createElement("dropdown");
    let option = document.createElement('options');
   for ( let j = 0; j < select.options.length; j++) {
   langagebox.placeholder = userInfo.label
   formDatalist.append(options)
   formDatalist.append(languagebox)
 }
} else (userInfo.type == "input")
   let textbox = document.createElement( "input" );
   textbox.type = userInfo.type
   textbox.placeholder = userInfo.label
   textbox.id = userInfo.id
   formDatalist.append(textbox)
 }
